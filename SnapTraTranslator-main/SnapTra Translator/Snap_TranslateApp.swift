...
final class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, NSMenuDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        // Pre-warm ScreenCaptureKit so any system dialog appears at launch
        // rather than on first hotkey press, after permission is already granted.
        if #available(macOS 12.3, *), CGPreflightScreenCaptureAccess() {
    ...
    private func configureStatusItem() {
        ...
        let toggleItem = NSMenuItem(title: "Translation: On", action: #selector(toggleTranslation), keyEquivalent: "")
        toggleItem.target = self
        toggleItem.tag = 100  // tag to identify for updates
        menu.addItem(toggleItem)
        menu.addItem(.separator())
        menu.addItem(withTitle: "Settings…", action: #selector(statusItemClicked), keyEquivalent: ",")
        menu.addItem(.separator())
        menu.addItem(withTitle: "Quit SnapTra Translator", action: #selector(quitApp), keyEquivalent: "q")
        for menuItem in menu.items {
            if menuItem.target == nil && !menuItem.isSeparatorItem {
                menuItem.target = self
            }
        }
        statusItem = item
        statusItem?.menu = nil  // menu shown only on right-click
    ...
    @objc private func statusItemClicked() {
        ...
        if event.type == .rightMouseUp {
            ...
            if let button = statusItem?.button, let menu = statusMenu {
                ...
                // Reset so left-click works normally next time
                statusItem?.menu = nil
            }
        } else {
            openSettings()
    ...
    nonisolated func menuNeedsUpdate(_ menu: NSMenu) {
        MainActor.assumeIsolated {
            if let toggleItem = menu.item(withTag: 100) {
                let state = model.isTranslationEnabled ? "On" : "Off"
                toggleItem.title = "Translation: \(state)"
            }
        }
    }
...
final class SettingsWindowController: NSWindowController {
    init(model: AppModel) {
        ...
        hostingView.frame = NSRect(x: 0, y: 0, width: 440, height: 720)
