...
final class OverlayWindowController: NSWindowController {
    ...
    /// When true, resizeToFit will only adjust height in-place (no re-clamping).
    ...
    init(model: AppModel) {
        ...
        // Allow user to resize, with reasonable min/max
        panel.minSize = NSSize(width: 200, height: 100)
        panel.maxSize = NSSize(width: 800, height: 1200)
        super.init(window: panel)
    }
    ...
    /// Enable or disable drag-to-move for the overlay window.
    func setDraggable(_ draggable: Bool) {
        window?.isMovableByWindowBackground = draggable
    }

    /// Check if the mouse cursor is currently inside the overlay window frame.
    func isMouseInsideOverlay() -> Bool {
    ...
    /// Resize the window to fit current content without moving the anchor position.
    /// Called when expand/collapse changes the content height.
    func resizeToFit() {
        guard let window, window.isVisible else { return }
        hostingView.layoutSubtreeIfNeeded()

        // Small delay to let SwiftUI re-layout after state change
        DispatchQueue.main.async { [weak self] in
            self?.performResize()
        }
        // Second delayed resize to catch WebView content height updates
    ...
    private func performResize() {
        ...
        // When pinned, skip auto-resize — let the user control the size.
        if isPinned { return }

        hostingView.layoutSubtreeIfNeeded()
        let newSize = hostingView.fittingSize
        let oldFrame = window.frame
        // Keep the top-left corner pinned while height changes
        let newOrigin = CGPoint(x: oldFrame.origin.x, y: oldFrame.maxY - newSize.height)
        ...
        origin.x = min(max(origin.x, screenFrame.minX + shadowMargin), screenFrame.maxX - newSize.width - shadowMargin)
        origin.y = min(max(origin.y, screenFrame.minY + shadowMargin), screenFrame.maxY - newSize.height - shadowMargin)

        let targetFrame = CGRect(origin: origin, size: newSize)
        NSAnimationContext.runAnimationGroup { context in
